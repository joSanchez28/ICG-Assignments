...
void Solar_viewer::paint()
{
	...
	if (!in_ship_) {
		...
		eye = mat4::rotate_y(y_angle_) * mat4::rotate_x(x_angle_) * vec4(0, 0, dist_factor_ * (*planet_to_look_at_).radius_, 0);
	...
	// observing ship therefore rotation around x-axis is fixed (i.e. ignore current value of x_angle)
		...
		up = mat4::rotate_y(ship_.angle_) * mat4::rotate_y(y_angle_) * up;
	}
...
void Solar_viewer::draw_scene(mat4& _projection, mat4& _view)
{
	...
	for (int i = 0; i < planets.size(); i++) {
		...
		m_matrix = mat4::translate((*planets[i]).pos_) * mat4::rotate_y((*planets[i]).angle_orbit_) *
			mat4::rotate_y((*planets[i]).angle_self_) * mat4::scale((*planets[i]).radius_);
